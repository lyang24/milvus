...
// Exercises the single-target fast path in ExecArrayContains which bypasses
// the MultiElement abstraction when vals_.size() == 1.
TEST(Expr, TestArrayContainsSingleTarget) {
    auto schema = std::make_shared<Schema>();
    auto i64_fid = schema->AddDebugField("id", DataType::INT64);
    auto long_array_fid =
        schema->AddDebugField("long_array", DataType::ARRAY, DataType::INT64);
    ...
    // --- ContainsAny with a one-element target list (also hits single-target
    //     fast path since vals_.size() == 1) ---
    {
        int64_t target = 100;
        auto check = [target](const std::vector<int64_t>& values) {
            return std::find(values.begin(), values.end(), target) !=
